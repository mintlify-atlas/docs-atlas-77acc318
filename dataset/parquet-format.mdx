...
The `parquet.py` script in the repository root regenerates all Parquet files from the source JSONL files. Run it from the `data/` directory.
...
The script reads every `chap*_dataset.jsonl` file, writes a corresponding `.parquet` file, and then writes a combined `full_chapters_dataset.parquet`.
...
<Warning>
  The script uses `glob.glob('chap*_dataset.jsonl')` and must be run from the `data/` directory. Running it from the repository root will produce no output files.
</Warning>
